...
def _call_module(module_path: str, dry_run: bool, yes: bool, log_path: Optional[str], targets: Optional[List[str]]):
    import importlib
...
def run_flow(flow_name: str, dry_run: bool = True, yes: bool = False, log_path: Optional[str] = None, targets: Optional[List[str]] = None) -> Union[Dict, List[Dict]]:
    ...
    res = _call_module(module_path, dry_run, yes, log_path, targets)
    # normalize
    if res is None:
        return []
